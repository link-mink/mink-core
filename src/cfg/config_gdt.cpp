    ...
    for(it = hndlr_map.begin(); it != hndlr_map.end(); ++it){
        ...
        if(it->second.children.size() > 0){
            ...
            for (unsigned int i = 0; i < it->second.children.size(); i++)
                it->second.children[i]->onc_hndlr_exec =
                    !it->second.children[i]->onc_hndlr_exec;
        }
    }
...
void config::CfgUpdateStreamDone::run(gdt::GDTCallbackArgs* args){
    ...
    for(unsigned int i = 0; i<snext->cfg_res.children.size(); i++){
        ...
        ch_item = (*snew->config->get_definition_root())(
            tmp_item->name.c_str(), true, tmp_item->node_type, true);
        // config path exists
        if(ch_item != NULL){
            // BLOCK node
...
config::GDTCfgNtfUser::GDTCfgNtfUser(gdt::GDTClient* _gdtc){
    gdtc = _gdtc;
}
...
config::GDTCfgNotification::GDTCfgNotification(std::string* _cfg_path) : CfgNotification(_cfg_path) {
    ready = false;
}

config::GDTCfgNotification::~GDTCfgNotification(){
...
void* config::GDTCfgNotification::reg_user(void* usr){
    GDTCfgNtfUser* uid = (GDTCfgNtfUser*)usr;
    if(!user_exists(uid)){
        users.push_back(*uid);
        return usr;
    }
...
bool config::GDTCfgNotification::user_exists(config::GDTCfgNtfUser* usr){
...
unsigned int config::GDTCfgNotification::get_user_count(){
    return users.size();
}
...
int config::replicate(const char* repl_line, gdt::GDTClient* _client, const char* _daemon_id, config::UserId* _cfg_user_id){
...
int config::notify_user(config::Config* config,
    ...
    if(cfg_flat != NULL && ntf_user != NULL){
        ...
        gdt::GDTStream* gdt_stream = ntf_user->gdtc->new_stream(
            (char*)ntf_user->user_type, (char*)ntf_user->user_id, NULL, snext);
...
int config::user_logout(config::Config* config,
                        gdt::GDTClient* cfgd_gdtc,
                        const char* _daemon_id,
                        config::UserId* cfg_user_id){
    if(config != NULL && cfgd_gdtc != NULL){
        ...
        // remove payload
        if(gdtm->_body->_conf->_payload != NULL) gdtm->_body->_conf->_payload->unlink(1);
        // set params
        if(gdtm->_body->_conf->_params == NULL){
            gdtm->_body->_conf->set_params();
            // set children, allocate more
...
int config::user_login(config::Config* config,
                       gdt::GDTClient* cfgd_gdtc,
                    ...
                    if(in_msg->_header->_status->has_linked_data(*in_sess)){
                        if(in_msg->_header->_status->linked_node->tlv->value[0] == 0){
                            ...
                            if (in_msg->_header->_source->_id != NULL) {
                                if (in_msg->_header->_source->_id
                                        ->has_linked_data(*in_sess)) {
                                    // C string
                                    memcpy(
                                        out_daemon_id,
                                        in_msg->_header->_source->_id
                                            ->linked_node->tlv->value,
                                        in_msg->_header->_source->_id
                                            ->linked_node->tlv->value_length);

                                    // null character
                                    out_daemon_id[in_msg->_header->_source->_id
                                                      ->linked_node->tlv
                                                      ->value_length] = 0;
                                }
                            }
...
int config::notification_request(config::Config* config,
    ...
    if(config != NULL && cfgd_gdtc != NULL && usr_root != NULL){
        ...
        // set event handlers
        if (cfgd_gdtc->get_callback(gdt::GDT_ET_STREAM_NEW) == NULL &&
            cfgd_gdtc->get_callback(gdt::GDT_ET_CLIENT_TERMINATED) == NULL) {
